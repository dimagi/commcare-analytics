...
class TestViews(HQDBTestCase):
    ...
    def test_datasource_upload(self, *args):
        ...
        with patch('hq_superset.views.trigger_datasource_refresh') as refresh_mock:
            ...
            client.get(
                f'/hq_datasource/update/{ucr_id}?name=ds1', follow_redirects=True
            )
        ...
        def _test_sync_or_async(ds_size, routing_method, user_id):
            with patch(
                ...
                refresh_mock.assert_called_once_with(
                    domain, ucr_id, ds_name, file_path, TEST_DATASOURCE, user_id
                )
    ...
    def test_refresh_hq_datasource(self, *args):
        ...
        with patch(
            ...
            def _test_upload(test_data, expected_output):
                ...
                self.assertEqual(
                    datasets['result'][0]['schema'], get_schema_name_for_domain('test1')
                )
